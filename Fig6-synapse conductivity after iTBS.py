...
simConfig.duration = 195*1e3        # Duration of the simulation, in ms  #72*1e3
...
def Simulation(amp_pre, amp_post):
    simConfig.filename='iTBS-pre-'+str(amp_pre)+'post-'+str(amp_post)
    # COUNT=0
...
for chunk in chunks:
    ...
    for i in range (chunk_size):
        ...
        filename='iTBS-pre-'+str(pre_a)+'post-'+str(post_a)

        voltage_cut=[]
        time_cut=[]
        for j in range  (3876800,3877000,1):
