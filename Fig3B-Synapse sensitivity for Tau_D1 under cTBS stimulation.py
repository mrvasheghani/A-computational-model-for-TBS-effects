...
netParams.synMechParams['STD']   = {'mod':'FDSExp2Syn','tau_F': tau_D1_input} #,'d1': 0.7 , 'tau_D2' : 8200}                            # STD synaptic mechanism   
###############################################################################  Population parameters
...
###############################################################################
###############################################################################
STDPparams = {'STDPon':1, 'hebbwt': 0.001, 'antiwt':-0.001, 'wmax': 50, 'RLon': 0 ,'tauhebb': 10, 'RLwindhebb': 0, 'useRLexp': 0, 'softthresh': 0, 'verbose':1}
...
simConfig.duration = 43*1e3# 43*1e3        # Duration of the simulation, in ms  #72*1e3
...
for amplitude1 in range (116,117,1):
    amplitude2=0
    def Simulation(tau_D1_input):
        simConfig.filename='cTBS-tau_D1-'+str(tau_D1_input)+'pre-'+str(amplitude1)+'post-'+str(amplitude2)
        # COUNT=0
        start_time=1000
        ...
        # print ('COUNT='+str(COUNT))
                    ...
                    netParams.synMechParams['STD']   = {'mod':'FDSExp2Syn','tau_D1': tau_D1_input} #,'d1': 0.7 , 'tau_D2' : 8200}
    ...
    ModelOutput=Parallel(n_jobs=num_cores)(delayed(Simulation)(tau_D1_input) for tau_D1_input in range (180,585,100))
...
plt.plot(Time, Data[0], label = "$\u03c4 $_D1=180")
plt.plot(Time, Data[1], label = "$\u03c4 $_D1=280")
plt.plot(Time, Data[2], label = "$\u03c4 $_D1=380")
plt.plot(Time, Data[3], label = "$\u03c4 $_D1=480")
plt.plot(Time, Data[4], label = "$\u03c4 $_D1=580")

plt.legend()
plt.xlabel('Time (ms)')
plt.ylabel('synapse conductivity')
plt.title('Synapse sensitivity for $\u03c4 $_D1 under cTBS')
plt.savefig('Synapse sensitivity for \u03c4_D1 under cTBS.png',dpi=600)
