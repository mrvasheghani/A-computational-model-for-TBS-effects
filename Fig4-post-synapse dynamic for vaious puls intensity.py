...
simConfig.analysis['plotTraces'] = {'include': [0,1], 'saveFig': True, 'oneFigPer':'trace'}  # Plot recorded traces for this list of cells
...
threshold=60.00      
for amplitude1 in range (106,127,10):   
    def Simulation(amplitude2):
        simConfig.filename='Spiking pattern in Pre and post synapse puls intensity-'+str(amplitude1)+'post-'+str(amplitude2)
        # COUNT=0
        start_time=0
        for ttt in range (0,200000,10000):
        ...
        response1 = sim.allSimData.V_soma.cell_0
        response2 = sim.allSimData.V_soma.cell_1
        SimDataTime=sim.simData.t
        return response1,response2,SimDataTime
    num_cores = multiprocessing.cpu_count()-1
    ModelOutput=Parallel(n_jobs=num_cores)(delayed(Simulation)(amplitude2) for amplitude2 in range (0,121,40))  
